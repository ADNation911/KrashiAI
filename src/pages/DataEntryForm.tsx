  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: 12 }}>
        ...
        <div>
          <span style={{ fontSize: 16, fontWeight: '700', color: colors.textPrimary, display: 'block' }}>
        ...
        <div style={{
          ...
          {fields.map((field) => (
            <div key={field.label} style={{
              ...
              <span style={{ fontSize: 13, fontWeight: '700', color: colors.primaryDark }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 12 }}>
        ...
        <button
          ...
          style={{
            ...
            fontSize: 14, fontWeight: '500', color: colors.textSecondary, cursor: 'pointer',
          }}
          onClick={onDismiss}
        >
          Dismiss
...
/* ──────────────────────────────────────────────
   ...
   ────────────────────────────────────────────── */
const ScanningOverlay: React.FC<{ previewUrl: string | null }> = ({ previewUrl }) => (
  <div style={{
    backgroundColor: colors.white,
    borderRadius: 16,
    padding: 16,
    ...
    {previewUrl && (
      <div style={{
        width: '100%', maxHeight: 160, borderRadius: 10, overflow: 'hidden',
        marginBottom: 12, position: 'relative',
      }}>
        <img
          src={previewUrl}
          alt="Scanning..."
          style={{ width: '100%', height: 160, objectFit: 'cover', filter: 'brightness(0.7)' }}
        />
        ...
        <div style={{
          ...
          <div className="scan-pulse" style={{
...
const DataEntryForm: React.FC = () => {
  ...
  /* ─────────────────────────────────────────
     ...
     ───────────────────────────────────────── */
  const handleScanSoilCard = useCallback(async (file: File) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSubmitResult({
        success: false,
        error: 'Could not reach the recommendation server. Please make sure the API server is running (node server.js in agrismart-server folder).',
      });
    } finally {
      ...
      <div style={{ flex: 1, paddingBottom: 60 }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', marginBottom: 10 }}>
            ...
            <span style={{ marginLeft: 8, fontSize: 16, fontWeight: '700', color: colors.primaryDark }}>
              Scan Soil Health Card
            </span>
          </div>
          <span style={{ fontSize: 13, color: colors.textSecondary, display: 'block', marginBottom: 12 }}>
            Upload or capture your Soil Health Card (मृदा स्वास्थ्य कार्ड) image to auto-fill soil parameters using AI
          </span>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10 }}>
            ...
            >
              ...
              <span style={{ marginLeft: 8, fontSize: 14, fontWeight: '600', color: colors.primary }}>
            ...
            <button
              ...
              style={{
                ...
                padding: '12px 16px', borderRadius: 12,
                backgroundColor: colors.primary,
                border: 'none', flex: 1, cursor: 'pointer',
            ...
            >
              ...
              <span style={{ marginLeft: 8, fontSize: 14, fontWeight: '600', color: colors.white }}>
                Camera
              </span>
            </button>
          </div>
        ...
        {scanError && (
          <div style={{
            ...
            <span style={{ fontSize: 13, color: '#E65100', flex: 1 }}>{scanError}</span>
            <button
              style={{ border: 'none', background: 'none', cursor: 'pointer', color: '#E65100', fontWeight: 600 }}
        ...
        >
          {isSubmitting ? (
            <>
              ...
              <span style={{ color: colors.white, fontSize: 16, fontWeight: '600', marginLeft: 8 }}>
        ...
        {submitResult && (
          <div id="recommendation-results" style={{ marginBottom: 24, animation: 'fadeIn 0.4s ease-out' }}>
            {submitResult.success === false ? (
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', marginBottom: 8 }}>
                  ...
                  <span style={{ marginLeft: 8, fontSize: 16, fontWeight: '700', color: '#E65100' }}>
                    Server Unavailable
                  </span>
                </div>
            ...
            ) : submitResult.recommendations && submitResult.recommendations.length > 0 ? (
              ...
              <div>
                <div style={{
                  ...
                  <div>
                    <span style={{ fontSize: 18, fontWeight: '700', color: colors.textPrimary, display: 'block' }}>
                      Crop Recommendations
                    </span>
                    <span style={{ fontSize: 12, color: colors.textSecondary }}>
                      Top {submitResult.recommendations.length} crops for your conditions
                      {submitResult.calculated_rainfall && ` • Rainfall: ${submitResult.calculated_rainfall}mm`}
                    </span>
                  </div>
                </div>

                {submitResult.recommendations.map((rec: any, idx: number) => {
                  const scoreColor = rec.score >= 70 ? colors.success : rec.score >= 50 ? '#F59E0B' : colors.error;
                  const medals = ['🥇', '🥈', '🥉'];
                  return (
                    <div key={idx} style={{
                      backgroundColor: colors.white,
                      ...
                      padding: 14,
                      marginBottom: 10,
                      boxShadow: idx === 0 ? '0 4px 16px rgba(18, 97, 68, 0.15)' : '0 2px 6px rgba(0,0,0,0.06)',
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                        <div style={{ display: 'flex', alignItems: 'center' }}>
                          <span style={{ fontSize: 22, marginRight: 8 }}>{medals[idx] || '🌱'}</span>
                          <div>
                            <span style={{ fontSize: 16, fontWeight: '700', color: colors.textPrimary, display: 'block' }}>
                              {rec.crop}
                            </span>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
                        <div style={{
                          backgroundColor: '#F0FDF4', borderRadius: 8, padding: '6px 10px',
                        }}>
                          <span style={{ fontSize: 10, color: colors.textSecondary, display: 'block' }}>Fertilizer</span>
                          <span style={{ fontSize: 12, fontWeight: '600', color: colors.primaryDark }}>{rec.fertilizer}</span>
                        </div>
                        <div style={{
                          backgroundColor: rec.rainfall_status === 'Sufficient' ? '#F0FDF4' : '#FFF7ED',
                          borderRadius: 8, padding: '6px 10px',
                        }}>
                          <span style={{ fontSize: 10, color: colors.textSecondary, display: 'block' }}>Rainfall</span>
                          <span style={{
                            fontSize: 12, fontWeight: '600',
                            color: rec.rainfall_status === 'Sufficient' ? colors.success : '#D97706',
                          }}>
                            {rec.rainfall_status}
                          </span>
                        </div>
              ...
              <div style={{
                ...
                border: '1px solid #FFD54F', textAlign: 'center',
              }}>
                <Icon name="info-outline" size={32} color="#F9A825" />
                <span style={{ fontSize: 14, color: '#F57F17', marginTop: 8, display: 'block' }}>
                  No matching crops found for current conditions. Try adjusting soil or weather parameters.
                </span>
              </div>
            )}
          </div>
